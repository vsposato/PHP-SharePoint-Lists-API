<?php
    ...
    class SharePointAPI {
         ...
         *
         * Provides a select field ready listing of CHOICES from a specific listing
         *
         * @param $list_name
         * @param $field_name
         *
         ...
         * @version 1.0
         */
        public function getChoiceFieldOptions( $list_name, $field_name ) {
        ...
        ';
            ...
            // Attempt the query to Sharepoint
            $rawXml = '';
            ...
            // Get nodes
            $returnChoiceArray = $this->_getArrayOfChoiceFieldOptions( $rawXml, $field_name );
        ...
        /**
         * getArrayOfChoiceFieldOptions
         *
         * Will return a list of options for the CHOICE fields from within Sharepoint
         *
         * @param $rawXml
         * @param $fieldName
         *
         ...
         * @version 1.0
         */
        private function _getArrayOfChoiceFieldOptions( $rawXml, $fieldName ) {
            // Get DOM instance and load XML
            $dom = new \DOMDocument();
            $dom->preserveWhiteSpace = FALSE;
            $dom->loadXML( $rawXml );

            // Item counter for iterating through our return
            $i = 0;

            // Return choice array
            $returnChoiceArray = array();

            // While we are getting an object back, we need to continue iterating - this is a sign
            // that there are more results
            while ( is_object( $node = $dom->getElementsByTagName( 'Field' )->item( $i ) ) ) {
                // Once the name attribute of the field equals the field we are looking for - process it
                if ( ( $node->getAttribute( "Name" ) == $fieldName ) && ( $node->getAttribute( "Type" ) == "Choice" ) ) {
                    // Get all of the CHOICE elements from the current node
                    $childNodes = $node->getElementsByTagName( 'CHOICE' );
                    // Iterate over the returned choices
                    foreach ( $childNodes as $childNode ) {
                        // Put the value of the node as both the key and value of the array
                        $returnChoiceArray[$childNode->nodeValue] = $childNode->nodeValue;
                    }
                }
                $i++;
            }
